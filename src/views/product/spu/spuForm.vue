...
let unSelectSaleAttr = computed(() => {
    //allSaleAttr 全部销售属性:颜色、版本、尺码
    //saleAttr 已有的销售属性:颜色、版本
...
const save = async () => {
    ...
    SpuParams.value.spuImageList = imgList.value.map((item: any) => {
        return {
            ...
            imgUrl: (item.response && item.response.data) || item.url // 新增图片要用远程图片地址
        }
    });
    //2:整理销售属性的数据
    ...
    if (result.code == 200) {
        ...
        //通知父组件切换场景为0 以及是添加还是修改来控制需不需要返回第一页
...
<template>
    <el-form label-width="100px">
        ...
        <el-form-item label="SPU图片">
            <!-- v-model:fileList->展示默认图片 要求数据格式 name、url
                 action:上传图片的接口地址
                 list-type:文件列表的类型
            -->
        ...
        <el-form-item label="SPU销售属性">
            ...
            <el-select v-model="saleAttrIdAndValueName"
                ...
                <!-- value变成想要的格式 收集到saleAttrIdAndValueName-->
            ...
            <el-table border style="margin:10px 0px" :data="saleAttr">
                ...
                <el-table-column label="销售属性值">
                    ...
                    <template #="{ row, $index }">
                        ...
                        <!-- 编辑 一开始row.flag为undefined == false显示button -->
